"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
const core_1 = require('@angular/core');
const event_1 = require('../../../shared/models/event');
const event_viewmodels_service_1 = require('../../../core/services/viewmodels-services/event-viewmodels.service');
let EventCardComponent = class EventCardComponent {
    constructor(eventViewModelsService) {
        this.eventViewModelsService = eventViewModelsService;
    }
    ngOnInit() {
        this.viewModelObservableSubscription = this.eventViewModelsService
            .getViewModel(this.event)
            .subscribe(vm => this.viewModel = vm);
    }
    ngOnDestroy() {
        this.viewModelObservableSubscription.unsubscribe();
    }
};
__decorate([
    core_1.Input(), 
    __metadata('design:type', event_1.Event)
], EventCardComponent.prototype, "event", void 0);
EventCardComponent = __decorate([
    core_1.Component({
        moduleId: module.id,
        selector: 'event-card',
        styleUrls: ['event-card.component.css'],
        templateUrl: 'event-card.component.html'
    }), 
    __metadata('design:paramtypes', [event_viewmodels_service_1.EventViewModelsService])
], EventCardComponent);
exports.EventCardComponent = EventCardComponent;
//# sourceMappingURL=event-card.component.js.map